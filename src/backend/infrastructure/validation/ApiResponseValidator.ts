...
export class ApiResponseValidator {
  ...
  static validate<T>(schema: z.ZodSchema<T>, data: unknown, context: string): T {
    try {
      ...
      streamDeck.logger.error(`Unexpected validation error in ${context}:`, error);
      throw new ApiValidationError(
   ...
   */
  static safeParse<T>(schema: z.ZodSchema<T>, data: unknown, context: string): T | null {
    try {
      return this.validate(schema, data, context);
    } catch (error) {
      streamDeck.logger.warn(`Safe validation failed in ${context}, returning null:`, error);
      return null;
    }
  }
  ...
  static validateArray<T>(
    ...
    data.forEach((item, index) => {
      try {
        const validItem = this.validate(itemSchema, item, `${context}[${index}]`);
        validItems.push(validItem);
      } catch (error) {
        errors.push(`Item ${index}: ${error instanceof Error ? error.message : 'Validation failed'}`);
      }
    });

    if (errors.length > 0) {
      streamDeck.logger.warn(`Some items failed validation in ${context}:`, errors);
    }
...
export class ApiValidationError extends Error {
  ...
  getUserFriendlyMessage(): string {
    ...
    const fieldPath = mainError.path.length > 0 ? mainError.path.join('.') : 'data';
