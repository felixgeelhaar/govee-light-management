...
export class LightControlService {
  ...
  async controlLight(
    ...
    if (!light.canBeControlled()) {
      throw new Error(`Light ${light.name} is offline and cannot be controlled`);
    }
    ...
    switch (action) {
      ...
      case 'colorTemperature':
        if (!(value instanceof ColorTemperature)) {
          throw new Error('ColorTemperature value is required for colorTemperature action');
        }
  ...
  async turnOnLightWithSettings(
    ...
    if (color && colorTemperature) {
      throw new Error('Cannot set both color and color temperature simultaneously');
    }
    ...
    if (color) {
      ...
      await this.lightRepository.turnOnWithColorTemperature(light, colorTemperature, brightness);
      light.updateState({ 
        isOn: true, 
        colorTemperature, 
        brightness, 
