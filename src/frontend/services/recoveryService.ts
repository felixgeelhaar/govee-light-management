...
import { createAppError, ErrorCodes, isRecoverableError } from '../utils/errorHandling'
...
export class RecoveryService {
   ...
   */
  async attemptRecovery(error: Error, context?: Record<string, any>): Promise<boolean> {
    ...
    if (applicableStrategies.length === 0) {
      console.log('No applicable recovery strategies found for error:', error.message)
      return false
    }
  ...
  async performHealthCheck(): Promise<{
    ...
    if (cacheHealth.status !== 'healthy') {
      issues.push(`Cache ${cacheHealth.status}: ${cacheHealth.recommendations.join(', ')}`)
