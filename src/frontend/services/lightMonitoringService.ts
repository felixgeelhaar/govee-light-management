...
export class LightMonitoringService {
  ...
  private changeCallbacks = new Map<string, ((change: LightStateChange) => void)[]>()
   ...
   */
  async startMonitoring(lightIds: string[], config?: Partial<MonitoringConfig>): Promise<void> {
  ...
  getOfflineLights(): LightState[] {
    ...
    return Array.from(this.lightStates.values()).filter(state => 
      !state.isOnline || (now - state.lastSeen > threshold)
    )
  }
   ...
   */
  onStateChange(deviceId: string, callback: (change: LightStateChange) => void): () => void {
  ...
  private async performPoll(): Promise<void> {
    ...
    await performanceService.timeAsync(operationId, 'Light State Poll', async () => {
      const lightIds = Array.from(this.monitoredLights)
  ...
  private updateLightStates(states: any[]): void {
    ...
    states.forEach((stateData: any) => {
      ...
      const newState: LightState = {
        ...
        lastUpdated: now,
        lastSeen: stateData.isOnline !== false ? now : (existingState?.lastSeen || now)
      }
  ...
  private detectAndRecordChanges(oldState: LightState, newState: LightState): void {
    const properties: (keyof LightState)[] = [
      'isOnline', 'powerState', 'brightness', 'color', 'colorTemperature'
    ]
...
export function useLightMonitoring() {
  return {
    // State
    isMonitoring: computed(() => lightMonitoringService.getMonitoringStats().isActive),
    monitoringStats: computed(() => lightMonitoringService.getMonitoringStats()),
    ...
    addLights: (lightIds: string[]) => lightMonitoringService.addLights(lightIds),
    removeLights: (lightIds: string[]) => lightMonitoringService.removeLights(lightIds),
    
    // Data access
    getLightState: (deviceId: string) => lightMonitoringService.getLightState(deviceId),
    getAllLightStates: () => lightMonitoringService.getAllLightStates(),
    getOfflineLights: () => lightMonitoringService.getOfflineLights(),
    getRecentChanges: (since?: number) => lightMonitoringService.getRecentChanges(since),
    
    // Events
    onStateChange: (deviceId: string, callback: (change: LightStateChange) => void) =>
