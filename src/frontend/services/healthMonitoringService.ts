...
export class HealthMonitoringService {
  ...
  private async updateAllMetrics(): Promise<void> {
    ...
    this.updateMetric('cache_hit_rate', {
      ...
      unit: '%',
      status: hitRate >= 75 ? 'healthy' : hitRate >= 50 ? 'warning' : 'critical',
      threshold: { warning: 50, critical: 25 },
      lastUpdated: now
    })
    ...
    if ((performance as any).memory) {
      ...
      memoryStatus = memoryUsage > 200 ? 'critical' : memoryUsage > 100 ? 'warning' : 'healthy'
    }
    ...
    const errorRate = recoveryStats.totalAttempts > 0 
  ...
  private async runCacheDiagnostics(): Promise<SystemDiagnostic[]> {
    ...
    diagnostics.push({
      category: 'Cache',
      test: 'Cache Health',
      result: cacheHealth.status === 'healthy' ? 'pass' : 
             cacheHealth.status === 'warning' ? 'warning' : 'fail',
  ...
  private async runPerformanceDiagnostics(): Promise<SystemDiagnostic[]> {
    ...
    if (bottlenecks.length > 0) {
      diagnostics.push({
        ...
        result: bottlenecks.some(b => b.severity === 'high') ? 'fail' : 'warning',
  ...
  private async runMemoryDiagnostics(): Promise<SystemDiagnostic[]> {
    ...
    if ((performance as any).memory) {
      ...
      diagnostics.push({
        category: 'Memory',
        test: 'Heap Usage',
        result: usagePercent > 80 ? 'fail' : usagePercent > 60 ? 'warning' : 'pass',
    ...
    diagnostics.forEach(diagnostic => {
      if (diagnostic.result === 'fail' || diagnostic.result === 'warning') {
        switch (diagnostic.category) {
          case 'WebSocket':
            recommendations.push('Check network connection and Stream Deck software')
            break
          case 'Cache':
            recommendations.push('Consider clearing cache or adjusting cache settings')
            break
          case 'Performance':
            ...
            break
          case 'Memory':
            recommendations.push('Consider reloading the Property Inspector to free memory')
            break
          case 'Monitoring':
            recommendations.push('Enable light monitoring for better system insights')
            break
        }
      }
    })
  ...
  private calculateTrend(): 'improving' | 'stable' | 'declining' {
    ...
    const previousAverage = this.healthHistory.slice(-10, -5)
...
export function useHealthMonitoring() {
  return {
    ...
    getRecentDiagnostics: (limit?: number) => healthMonitoringService.getRecentDiagnostics(limit),
