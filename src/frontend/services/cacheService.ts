...
export class CacheService {
  ...
  getHealthStatus(): {
    ...
    if (sizeUsage > 90) {
      ...
      recommendations.push(
        "Cache size critical - consider increasing maxSize or reducing TTL",
      );
    ...
    if (memoryUsage > 90) {
      status = "critical";
      recommendations.push(
        "Memory usage critical - consider reducing cache size",
      );
    ...
    if (stats.hitRate < 50 && this.stats.totalRequests > 100) {
      recommendations.push(
        "Low hit rate - consider adjusting TTL or cache strategy",
      );
    }
  ...
  private evictIfNeeded(): void {
    ...
    const stats = this.getStats();
    while (
      stats.memoryUsage > this.options.maxMemory &&
      this.accessQueue.length > 0
    ) {
      const lruKey = this.accessQueue.shift();
      if (lruKey) {
