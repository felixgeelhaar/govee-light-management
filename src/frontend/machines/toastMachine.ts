...
const canShowToast = (context: ToastMachineContext, event: any) => {
  ...
  if (toast.category) {
    const categoryLimit =
      context.categoryLimits.get(toast.category) || Infinity;
    const categoryCount = Array.from(context.activeToasts.values()).filter(
      (t) => t.category === toast.category,
    ).length;
...
const shouldReplaceExistingToast = (
...
export const toastMachine = setup({
  ...
  guards: {
    canShowToast: ({ context, event }) => {
      ...
      if (toast.category) {
        const categoryLimit =
          context.categoryLimits.get(toast.category) || Infinity;
        const categoryCount = Array.from(context.activeToasts.values()).filter(
          (t: ToastContext) => t.category === toast.category,
        ).length;
    ...
    canProcessQueue: ({ context }) =>
      context.activeToasts.size < context.maxActive,
  },
  actions: {
    showToast: assign(({ context, event }) => {
      ...
      // If we're at capacity, remove lowest priority toast
        ...
        newActiveToasts.size >= context.maxActive &&
        toast.priority &&
        toast.priority >= context.priorityThreshold
      ) {
        const lowestPriority = Array.from(newActiveToasts.entries()).sort(
          ([_, a], [__, b]) => (a.priority || 0) - (b.priority || 0),
        );
    ...
    queueToast: assign(({ context, event }) => {
      ...
      const newQueue = [...context.queue, queuedToast].sort(
    ...
    dismissCategory: assign(({ context, event }) => {
      ...
      const newQueue = context.queue.filter(
    ...
    processQueue: assign(({ context }) => {
      ...
      while (newActiveToasts.size < context.maxActive && newQueue.length > 0) {
        ...
        if (nextToast.category) {
          const categoryLimit =
            context.categoryLimits.get(nextToast.category) || Infinity;
          const categoryCount = Array.from(newActiveToasts.values()).filter(
            (t) => t.category === nextToast.category,
          ).length;
...
export class ToastService {
  ...
  private recentToasts: Map<
    string,
    { count: number; lastShown: number; id: string }
  > = new Map();
