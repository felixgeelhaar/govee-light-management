...
import { toastService, ToastContext, ToastType } from '../machines/toastMachine';
...
export function useToastMachine(): UseToastMachine {
  ...
  // Convenience methods
  const showSuccess = (title: string, message?: string, category?: string): string => {
    return toastService.showSuccess(title, message, category);
  };
  
  const showError = (title: string, message?: string, category?: string): string => {
    return toastService.showError(title, message, category);
  };
  
  const showWarning = (title: string, message?: string, category?: string): string => {
    return toastService.showWarning(title, message, category);
  };
  
  const showInfo = (title: string, message?: string, category?: string): string => {
  ...
  const showApiConnectionSuccess = (message?: string, category = 'api-connection'): string => {
  ...
  const showApiConnectionError = (message?: string, category = 'api-connection'): string => {
    return toastService.showApiConnectionError(message, category);
  };
  
  // Light testing methods
  const showLightTestStart = (lightName?: string): string => {
    const message = lightName ? `Testing ${lightName}...` : 'Sending command to light';
    return showToast({
      type: 'info',
      title: 'Testing Light',
      message,
...
export function useFeedback() {
  ...
  return {
    ...
    showInfo: (title: string, message?: string) => toast.showInfo(title, message),
    showSuccessToast: (title: string, message?: string) => toast.showSuccess(title, message),
    showErrorToast: (title: string, message?: string) => toast.showError(title, message),
    showWarningToast: (title: string, message?: string) => toast.showWarning(title, message),
